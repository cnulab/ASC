...
class IResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.minusscalar0=torch.nn.Parameter(torch.ones(1)*127.5000,requires_grad=False)
        self.mulscalar0=torch.nn.Parameter(torch.ones(1)*0.0078,requires_grad=False)


        self.conv0 = nn.Conv2d(3, self.inplanes, kernel_size=(3,3), stride=(1,1), padding=1, bias=False)
        self.bn0 = nn.BatchNorm2d(self.inplanes, eps=2e-05,momentum=0.9)
        ...
        self.bn1 = nn.BatchNorm2d(512 * block.expansion, eps=2e-05,momentum=0.9)
    ...
    def forward(self, x):
        with torch.cuda.amp.autocast(self.fp16):
            x= (x-self.minusscalar0)*self.mulscalar0
            x = self.conv0(x)
            x = self.bn0(x)
            x = self.prelu(x)
            x = self.layer1(x)
