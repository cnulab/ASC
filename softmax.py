...
def training(args):
    ...
    save_dir = Dir.mkdir("softmax_"+backbone)
    ...
    for epoch_i in range(1,epochs+1):
        ...
        if max_acc < acc:
            ...
            max_acc = acc
            save_model(model, os.path.join(save_dir, "softmax_"+backbone+"_best_model.pkl"))
        else:
            logger.info("validation acc did not improve from %.6f" % float(max_acc))

    save_model(model, os.path.join(save_dir, "softmax_"+backbone+"_final_model.pkl"))
